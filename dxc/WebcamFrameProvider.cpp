...
BGRAFrame *WebcamFrameProvider::GetFrame(ID3D11DeviceContext * devcon)
{
	...
	GetTextureDimensions(&current_texture_width, &current_texture_height);
...
UINT WebcamFrameProvider::GetWidth()
{
	UINT width = 0;
	GetTextureDimensions(&width, NULL);
	if (!width) {
		return 640;
	}
	return width;
...
UINT WebcamFrameProvider::GetHeight()
{
	UINT height = 0;
	GetTextureDimensions(NULL, &height);
	if (!height) {
		return 480;
	}
	return height;

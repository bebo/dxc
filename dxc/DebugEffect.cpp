...
//TODO: not linked errors
UINT DebugEffect::GetWidth()
{
	return inputFP->GetWidth();
}

//TODO: not linked errors
UINT DebugEffect::GetHeight()
{
	return inputFP->GetWidth();
}
...
void DebugEffect::Prepare()
{
	...
	BGRAFrame *frame = inputFP->GetFrame(devcon);
	ID3D11Texture2D *fpTex = frame->texture;
	delete frame;
...
BGRAFrame * DebugEffect::DrawFrame()
{
	devcon->Draw(6, 0);
	return new BGRAFrame(outputTexture);
}
...
void DebugEffect::AddInputFrameProvider(FrameProvider<BGRAFrame>* input)
{
	inputFP = input;
}
